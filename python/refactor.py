...
def rs(spots, pists, f: float, d: float, lam: float, res: int):
    ...
    slm_p_phase = 2.0*np.pi/(lam*(f*10.0**3)) * (spots[:, [0]] * slm_xcoord[pup_coords] + spots[:, [1]] * slm_ycoord[pup_coords]) + (np.pi * spots[:, [2]]) /(lam*(f*10.0**3)**2)*(slm_xcoord[pup_coords]**2+slm_ycoord[pup_coords]**2)
...
def wcsgs(spots, pists, f: float, d: float, lam: float, res: int, iters: int, sub: float, seed: int):
    ...
    weights=np.ones(spots.shape[0])/float(spots.shape[0])*(np.mean(np.sqrt(ints))/np.sqrt(ints))
    weights=weights/np.sum(weights)
