...
If used for academic purposes, please consider citing the appropriate literature (https://doi.org/10.3389/fncel.2021.609505, https://doi.org/10.3390/mps2010002))""")
        ...
        if screenID!=None:
            # @CHECK: what if I pass a None self.screenID?
        ...
        ## why do we need 4 bytes per pixel??
    ...
    def rs(self, spots_coords, spots_ints, get_perf=False):
        ...
        self.out, _ = csgs.rs(spots_coords[:,0], spots_coords[:,1], spots_coords[:,2], FOCAL_LENGTH, PITCH, WAVELENGTH, PIXELS, SEED)
        ...
        # # this is the only kernel invocation
        ...
        # this function is supposed to write to a texture
        self.draw_hologram()
    ...
    def draw_hologram(self):
        # I think we have to manually write the pixels of the texture, but where?
        ...
        buffer = ((self.out + np.pi) / (2.0 * np.pi)) * (self.lut_edges[1] - self.lut_edges[0]) + self.lut_edges[0]

        print(buffer.shape)
        print(self.screenresolution[1], self.screenresolution[0])
