	...
	#pragma omp parallel num_threads(OMP_NUM_THREADS)
	{
		for (int it = 0; it < iterations; ++it) {
			...
			for (int index = 0; index < pupil_count; ++index) {
				...
				for (int ispot = 0; ispot < n; ++ispot) {
					const double p_phase = COMPUTE_P_PHASE(WAVELENGTH, FOCAL_LENGTH, spots[ispot], x, y);
					const std::complex<double> spot_field_contribution = CEXP(total_phase - p_phase);

					#pragma omp atomic
					*(reinterpret_cast<double *>(spot_fields + ispot)) += spot_field_contribution.real();

					#pragma omp atomic
					*(reinterpret_cast<double *>(spot_fields + ispot) + 1) += spot_field_contribution.imag();
				}
			}
...
void gs_kernel_atomic_private_reordered(
	const	int			n,
	const	Point3D			spots[],
		double			pists[],
		std::complex<double>	spot_fields[],
